...
typedef struct AdderGeneratorEnv_s {
    SignalSourceMono_f* signal_a;
    SignalSourceMono_f* signal_b;
} AdderGeneratorEnv;

int adder_generator(float* sample, void* environment) {

    AdderGeneratorEnv* vars = (AdderGeneratorEnv*)environment;
...
void adder_deleter(void* environment) {
...
SignalSourceMono_f* new_adder(SignalSourceMono_f* signal_a, SignalSourceMono_f* signal_b) {

    AdderGeneratorEnv* environment = (AdderGeneratorEnv*)malloc(sizeof(AdderGeneratorEnv));
    ...
    SignalSourceMono_f* signal_out = new_ssmf(adder_generator); //, multiplier_deleter, environment);
